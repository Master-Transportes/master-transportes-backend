...
describe("AccessService", () => {
  ...
  afterAll(async () => {
    for (const userId of createdUserIds) {
      await userService.logoutAll(userId).catch(() => {});
    }
  ...
  describe("refreshSession()", () => {
    ...
    it("rejects refresh with wrong refresh token", async () => {
      const error = await userService.refreshSession("a".repeat(64)).catch((err: unknown) => err);
      expect(error).toBeInstanceOf(APIError);
      expect((error as APIError).code).toBe(ErrCode.Unauthenticated);
    });
    ...
    it("rejects refresh after session is revoked", async () => {
      ...
      const decoded = verifyToken(result.accessToken);
      await userService.logout(decoded.sid);

      const error = await userService.refreshSession(result.refreshToken).catch((err: unknown) => err);
      expect(error).toBeInstanceOf(APIError);
      expect((error as APIError).code).toBe(ErrCode.Unauthenticated);
    });
  });
  ...
  describe("refresh token rotation", () => {
    it(
      ...
      async () => {
        ...
        const refreshed = await userService.refreshSession(oldRefreshToken);
        expect(refreshed.refreshToken).not.toBe(oldRefreshToken);

        const error = await userService.refreshSession(oldRefreshToken).catch((err: unknown) => err);
        expect(error).toBeInstanceOf(APIError);
        expect((error as APIError).code).toBe(ErrCode.Unauthenticated);
      },
      { timeout: 15000 },
  ...
  describe("logout / revoke", () => {
    it("revokes all sessions on logoutAll", async () => {
      ...
      await userService.logoutAll(mainUserId);

      const countAfter = await sessionStore.count(mainUserId);
      expect(countAfter).toBe(0);

      const refreshError = await userService.refreshSession(session2.refreshToken).catch((err: unknown) => err);
  ...
  describe("full registration + login + refresh flow", () => {
    it("allows a newly registered client to sign in, refresh, and logout", async () => {
      ...
      const logoutRefreshError = await userService.refreshSession(refreshed.refreshToken).catch((err: unknown) => err);
  ...
  describe("getMe()", () => {
    it("returns full profile for the authenticated user", async () => {
      const profile = await userService.getMe(mainUserId);
    ...
    it("throws notFound for a non-existent user ID", async () => {
      const error = await userService.getMe("00000000-0000-0000-0000-000000000000").catch((err: unknown) => err);
      expect(error).toBeInstanceOf(APIError);
